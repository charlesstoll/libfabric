...
/**
 * @brief initialize the base fields of an explicit AV entry
 *
 * Set the base fields (fi_addr, AH) on an entry already inserted into the
 * explicit util AV and register it in the addr map. The caller allocates both
 * the util AV entry and the AH and owns their teardown on failure. Caller must
 * hold util_domain.lock and util_av.lock.
 *
 * @param[in]	av	efa address vector
 * @param[in]	entry	explicit AV entry allocated from the util AV
 * @param[in]	ah	address handle allocated for the entry
 * @param[in]	fi_addr	fi_addr assigned to the entry
 * @return	0 on success, a negative fabric errno on failure
 */
	...
	OFI_TSA_REQUIRES(efa_util_domain_lock_sym)
{
	...
	if (efa_av_entry_base_construct(av, efa_av_entry, ah, fi_addr))
		goto err_release_ah;
